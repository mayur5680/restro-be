...
class MockResto365JobService {
  async create() {
    // Mock the creation of a resto365 job
    return { id: 1 };
  }
}
...
describe('Resto365RestaurantService', () => {
  ...
  const mockCreateRestaurantDto = {
    ...
    shortRestaurantName: 'Test',
    fullRestaurantName: '',
    code: '',
    newCode: '',
    acronym: '',
    openingDate: '',
    refurbishmentStartDate: '',
    refurbishmentEndDate: '',
    freeBurritos: '',
    closingDate: '',
    address1: '',
  ...
  describe('findOneByBhyveId', () => {
    it('should return a restaurant by bhyveId', async () => {
      const mockRestaurant = new Resto365Restaurant();
      mockRestaurant.bhyveId = '123';
      jest.spyOn(repository, 'findOne').mockResolvedValue(mockRestaurant);

      const result = await service.findOneByBhyveId('123');
      expect(result).toEqual(mockRestaurant);
    });
    ...
    it('should throw NotFoundException if restaurant not found', async () => {
      ...
      await expect(service.findOneByBhyveId('123')).rejects.toThrowError();
    });
  });

  describe('findAllStates', () => {
    it('should return an array of states', async () => {
      const mockRestaurants = [{ state: 'state1' }, { state: 'state2' }].map(
        (restaurant) => Object.assign(new Resto365Restaurant(), restaurant),
      );
      jest.spyOn(repository, 'find').mockResolvedValue(mockRestaurants);
    ...
    it('should return an empty array if no states are found', async () => {
      jest.spyOn(repository, 'find').mockResolvedValue([]);
      const result = await service.findAllStates();
      expect(result).toEqual([]);
    });
  ...
  describe('findAllpriceTiers', () => {
    it('should return an array of price tiers', async () => {
      const mockRestaurants = [
        { priceTier: 'tier1' },
        { priceTier: 'tier2' },
      ].map((restaurant) =>
        Object.assign(new Resto365Restaurant(), restaurant),
      );
      jest.spyOn(repository, 'find').mockResolvedValue(mockRestaurants);

      const result = await service.findAllpriceTiers();
      expect(result).toEqual(['tier1', 'tier2']);
    });

    it('should return an empty array if no price tiers are found', async () => {
      jest.spyOn(repository, 'find').mockResolvedValue([]);
      const result = await service.findAllpriceTiers();
      expect(result).toEqual([]);
    });
  ...
  describe('update', () => {
    it.skip('should update a restaurant', async () => {
      ...
      const result = await service.update(
        1,
        {} as UpdateResto365RestaurantDto,
        mockRestoUser,
        {} as AuditParams,
        mockCorrelationId,
      );
    ...
    it('should throw an error if restaurant update fails', async () => {
      jest.spyOn(repository, 'save').mockRejectedValue(new Error());
      await expect(
        service.update(
          1,
